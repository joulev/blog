...
export function parseSearchQuery(query) {
  let tags = new Set();
  let words = new Set();
  query = query.toLowerCase().replace(/^"[^"]*"$/gm, str => {
    const tmpStr = str.replace(/"/g, "");
    if (tmpStr.length !== 0) words.add(tmpStr.replace(/</g, "&lt;").replace(/>/g, "&gt;"));
    return "";
  });
  query.split(" ").map(word => {
    const thisWord = word.replace(/[,\.]/g, "");
    if (thisWord === "" || thisWord === "tag:") return;
    if (thisWord.indexOf("tag:") === -1) words.add(thisWord.replace(/</g, "&lt;").replace(/>/g, "&gt;"));
...
function filterPostWords(post, words) {
  ...
  console.log(preview.replace("<", "&lt;").replace(">", "&gt;"));
  preview = preview.replace("<", "&lt;").replace(">", "&gt;").replace(new RegExp(`(${wordsEscaped.join("|")})`, "gi"),
