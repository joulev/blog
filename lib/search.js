...
function parseSearchQuery(query) {
  ...
  query.split(" ").map(word => {
    ...
    if (thisWord.indexOf("tag:") === -1) words.add(thisWord.toLowerCase());
    else tags.add(thisWord.substring(4).toLowerCase());
  });
...
const postHasWords = (post, words) => {
  for (let i = 0; i < words.length; i++) {
    if (!postHasWord(post, words[i])) return false;
  }
  return true;
}
/**
...
function filterPostsWords(posts, words) {
  ...
  return posts.filter(post => postHasWords(post, words));
}
