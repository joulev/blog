 ...
 * @returns An array indicating the starting and ending point of all occurences,
 ...
 * @example {
 *   count: 5,         // total number of occurences (including the title)
...
function filterPostWords(post, words) {
  const titleCount = getWordsOccurences(post.title, words).length;
  const matchArr = getWordsOccurences(post.plain, words);
  const previewInfo = getPreviewInfo(post.plain, matchArr);
  let preview = post.plain.substring(previewInfo.start, previewInfo.end);
  words.map(word => preview = addHeadAndTailToString(preview, word,
    '<span class="font-bold underline text-secondary-light dark:text-secondary-dark">', '</span>'));
  if (previewInfo.start !== 0) preview = "…" + preview;
  if (previewInfo.end !== post.plain.length - 1) preview = preview + "…";
  return {
    count: matchArr.length + titleCount,
